...
impl EidolonMcpServer {
    ...
    pub(crate) async fn handle_route_action(&self, params: serde_json::Value) -> serde_json::Value {
            ...
            let (strategy, reason) = if trauma_safety == 0.0 {
                ("ASK_USER", "Tool execution blocked by TraumaRegistry. Mandatory user intervention required.")
    ...
    pub(crate) async fn handle_sense_intent(&self, params: serde_json::Value) -> serde_json::Value {
            ...
            if let Some(ref engine) = *self.embedding_engine {
                ...
                if let Ok(query_vec) = engine.embed(query) {
...
Query: "{}" → "#,
            ...
            let confidence_calibrated = clamp01(
                confidence_raw * (0.78 + (1.0 - directional_uncertainty) * 0.22)
                    - short_query_penalty,
            );
    ...
    pub(crate) async fn handle_check_pattern(&self, params: serde_json::Value) -> serde_json::Value {
    ...
    pub(crate) async fn handle_simulate_response(&self, params: serde_json::Value) -> serde_json::Value {
            ...
            let signal_pressure = clamp01(
                leverage_signal * 0.5
                    + disable_stop_loss_signal * 0.7
                    + profile_sensitivity * 0.35,
            );
            ...
            let expected_loss = clamp01(
                best_prob * best_loss + base_prob * base_case_loss + worst_prob * worst_loss,
            );
    ...
    pub(crate) async fn handle_commit_pattern(&self, params: serde_json::Value) -> serde_json::Value {
            let pattern = params["pattern"].as_str().unwrap_or("unknown_pattern");
            let mut thermo = self.thermo.lock().await;
            let mut state = nalgebra::DVector::from_element(5, 0.5);
