...
use rusqlite::{params, Connection};
    ...
    fn setup_server_with_paths(
        users_path: PathBuf,
        telemetry_db_path: PathBuf,
    ) -> EidolonMcpServer {
        let oracle = DeepSeekOracle::new("dummy".to_string());
        EidolonMcpServer::with_storage_paths(oracle, users_path, telemetry_db_path)
    }
    ...
    async fn test_contract_tools_list_has_dual_stack_required_tools() {
        ...
        for required in LEGACY_COMPAT_TOOL_CATALOG {
            assert!(
                ...
                "missing required legacy tool in tools/list: {}",
                required
            );
        }
    ...
    fn test_evaluate_tool_promotion_thresholds() {
        ...
        let (pass_ok, pass_reasons) =
            EidolonMcpServer::evaluate_tool_promotion(&passing, &thresholds);
        assert!(pass_ok);
        ...
        let (fail_ok, fail_reasons) =
            EidolonMcpServer::evaluate_tool_promotion(&failing, &thresholds);
        assert!(!fail_ok);
