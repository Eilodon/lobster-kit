...
impl EidolonMcpServer {
    pub(crate) async fn handle_reason_chain(&self, params: serde_json::Value) -> serde_json::Value {
        ...
        for (index, strategy) in branch_templates.iter().take(branch_count).enumerate() {
            ...
            let branch_score =
                clamp01(coherence_score * 0.6 + (1.0 - critic_penalty) * 0.35 + exploration_bias);
        ...
        let verifier_score = clamp01(
            best_branch_score * 0.55 + (1.0 - critic_penalty) * 0.25 + groundedness_coverage * 0.20,
        );
        ...
        let mut used_evidence_id_list: Vec<String> = used_evidence_ids.into_iter().collect();
    ...
    pub(crate) async fn handle_recall_similar(
        &self,
        ...
        let mut scored: Vec<(f64, &MemoryEntry)> = mems_snapshot
            .iter()
            .map(|m| {
                let similarity = if !m.embedding.is_empty() && m.embedding.len() == query_vec.len()
                {
    ...
    pub(crate) async fn handle_memory_query(&self, params: serde_json::Value) -> serde_json::Value {
        ...
        let episodic_results = Self::episodic_memory_search(&memories_snapshot, query, limit);
                ...
                if has_causal_intent && !causal_results.is_empty() {
                    ...
                    let mut ranked_candidates: Vec<(String, f64)> = route_candidates
                        ...
                        .map(|(index, route)| {
                            ...
                            let prior_bonus =
                                ((route_candidates.len().saturating_sub(index)) as f64) * 0.03;
                            ...
                            (
                                route.to_string(),
                                clamp01(quality + prior_bonus + causal_boost + feedback_boost),
                            )
                        })
                        .collect();
                    ranked_candidates
    ...
    pub(crate) async fn handle_compress_context(
        &self,
        params: serde_json::Value,
    ) -> serde_json::Value {
        ...
        let result_json = tokio::task::spawn_blocking(move || {
            ...
            let mut candidates: Vec<(usize, String, usize, f64, HashSet<String>)> = sentences
                .iter()
                .enumerate()
                .map(|(index, sentence)| {
            ...
            let mut selected: Vec<(usize, String, usize, f64, HashSet<String>, bool)> = Vec::new();
            ...
            if selected.is_empty() {
                if let Some((index, sentence, _, score, token_set)) = candidates.first() {
                    ...
                    selected.push((*index, truncated, words, *score, token_set.clone(), false));
                    selected_words = words;
                }
            }

            let recent_indexes: Vec<usize> =
                (sentences.len().saturating_sub(preserve_recent)..sentences.len()).collect();
        ...
        .await
        .unwrap_or_else(|_| serde_json::json!({"error": "context_compression_task_panicked"}));
