import { DecisionLog, DEFAULT_WEIGHTS as REASONING_WEIGHTS, QTable, Q_CONFIG, MarketState, ActionType, QStateHash } from './types/EidolonTypes';
...
export class ActiveLearning {
  ...
  private applyAdamUpdate(category: keyof typeof this.weights, key: string, gradient: number): void {
    ...
    const stateKey = `${category}.${String(resolvedKey)}`;
  ...
  public reinitWasmGraphs(): void {
    this.cognitiveBrain.reinitWasmGraph(); // Wrapper for CausalBrain method
  }
  ...
  public getCausalSignal(
    ...
    // Map MarketState to SentinelVariables
    if (state.whaleFlow === 'ACCUMULATING') {
      activeSignals.push({ cause: 'WhaleNetFlow', effect: 'PriceDelta', direction: 1, label: 'Whales Buying', weight: 1.5 });
    } else if (state.whaleFlow === 'DUMPING') {
      activeSignals.push({ cause: 'WhaleNetFlow', effect: 'PriceDelta', direction: -1, label: 'Whales Dumping', weight: 1.5 });
    }

    if (state.gasPrice === 'HIGH') {
      activeSignals.push({ cause: 'GasPriceGwei', effect: 'Volatility', direction: 1, label: 'High Gas/Congestion', weight: 0.8 });
    }

    // New signals
    if (state.sentiment === 'EUPHORIC') {
      activeSignals.push({ cause: 'Sentiment', effect: 'PriceDelta', direction: 1, label: 'Euphoria', weight: 1.2 });
    }
    ...
    for (const signal of activeSignals) {
      // Ask Causal Brain: P(Effect | Cause) vs P(Effect)
      const prob = this.cognitiveBrain.getPrediction(signal.cause, signal.effect).prob;

      // If P(Effect|Cause) > 0.6, we have a strong causal link
      if (prob > 0.6) {
        // Boost confidence
        confidenceDelta += (prob - 0.5) * 20 * signal.weight;
        explanations.push(`${signal.label} -> ${signal.effect} (P=${prob.toFixed(2)})`);
      } else if (prob < 0.4) {
        // Reduce confidence (Counter-intuitive signal)
        confidenceDelta -= (0.5 - prob) * 20 * signal.weight;
        explanations.push(`Contra: ${signal.label} -/> ${signal.effect} (P=${prob.toFixed(2)})`);
      }
    }
  ...
  private pruneMemory(): void {
    ...
    if (keys.length > 5000) {
      this.debug('🧹 Pruning Q-Table Memory...');
      // Remove oldest 1000
      // Since it's a map, we just take first 1000 keys (approx LRU if insertion order preserved)
      for (let i = 0; i < 1000; i++) {
        delete this.qTable[keys[i]];
      }
    }
  }
}
