    ...
    private async runRpcOutage(): Promise<ChaosAlarm> {
        ...
        try {
            ...
            return this.alarm(
                ...
                true,
                'CRITICAL',
                `RPC unavailable or timed out after ${elapsed}ms (${ChaosHarness.messageOf(err)})`,
    ...
    private async runOracleDivergence(): Promise<ChaosAlarm> {
        try {
            ...
            return this.alarm(
                ...
                true,
                'HIGH',
                `Oracle check failed (${ChaosHarness.messageOf(err)})`,
    ...
    private async runMempoolSpike(): Promise<ChaosAlarm> {
        try {
            ...
            return this.alarm(
                ...
                true,
                'MEDIUM',
                `Mempool probe failed (${ChaosHarness.messageOf(err)})`,
