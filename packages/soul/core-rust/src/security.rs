...
impl ValueInvariant {
    ...
    pub fn check_invariant(&self, trade_value_usd: f64, predicted_impact: f64) -> Result<JsValue, JsValue> {
        ...
        if predicted_impact.is_nan() || !predicted_impact.is_finite() {
            return serde_wasm_bindgen::to_value(&InvariantCheckResult {
                safe: false,
                reason: Some("INVALID_INPUT: Predicted impact is NaN or Inf — failing safe".to_string()),
...
impl AntiRug {
    ...
    pub fn compute_score(&self, token_address: &str, data: JsValue) -> Result<JsValue, JsValue> {
        ...
        // 4. Major Risks
        if security_data.honeypot_with_same_creator { score -= PENALTY_HONEYPOT_CREATOR; }
        if security_data.cannot_buy { score -= PENALTY_CANNOT_BUY; }
        if security_data.is_proxy { score -= PENALTY_PROXY; }
        if security_data.is_mintable { score -= PENALTY_MINTABLE; }
        if !security_data.is_open_source { score -= PENALTY_NOT_OPEN_SOURCE; }

        // 5. Tax Penalties (for tokens within SOFT-HARD range)
        if buy_tax > TAX_THRESHOLD_SOFT { score -= (buy_tax.round() as i16) * PENALTY_TAX_PER_PCT; }
        if sell_tax > TAX_THRESHOLD_SOFT { score -= (sell_tax.round() as i16) * PENALTY_TAX_PER_PCT; }

        // 6. Ownership penalty
        if !renounced { score -= PENALTY_OWNER_NOT_RENOUNCED; }
        ...
        if let Some(is_locked) = security_data.liquidity_locked {
            ...
            if !is_locked { score -= PENALTY_LIQUIDITY_NOT_LOCKED; }
            if is_locked  { score += BONUS_LIQUIDITY_LOCKED; }
        } else {
            score -= PENALTY_LIQUIDITY_UNKNOWN;
        }

        // 7. Positive Signals (Bonuses)
        if security_data.is_whitelisted { score += BONUS_WHITELISTED; }
        if security_data.is_open_source && renounced { score += BONUS_OPEN_SOURCE_AND_RENOUNCED; }
        ...
        self.create_score(score, false, liquidity_locked, liquidity_unknown, security_data.is_open_source, renounced, status)
    }
