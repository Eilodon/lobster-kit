...
export class EmotionalCore {
  ...
  constructor(storage?: AppendOnlyAdapter) {
    ...
    this.breathEngine = new BreathEngine(BioParametersConfig.breathing.baseBPM);
  ...
  private setupEventListeners() {
    ...
    const unsubPrice = this.bus.subscribe(EidolonEventType.PRICE_UPDATE, () => {
      if (this.disposed) return;
  ...
  async tick(marketVolatility: number = 0.1, dt?: number) {
    ...
    const targetBPM = BioParametersConfig.breathing.baseBPM + (this.state.arousal * (BioParametersConfig.breathing.maxBPM - BioParametersConfig.breathing.baseBPM));
  ...
  private processBiologicalFunction(dt: number) {
    ...
    this.state.dopamine = Math.max(BioParametersConfig.limits.minHormoneLevel, this.state.dopamine - BioParametersConfig.metabolism.dopamineDecayRate * dt);
    ...
    const activeStress = this.state.volatility > volatilityDeadZone
      ? (this.state.volatility - volatilityDeadZone) * BioParametersConfig.metabolism.cortisolSpikeFactor
      : 0;
    const starvationStress = this.state.glucose < BioParametersConfig.thresholds.starvationGlucose ? 3.0 : 0;
    ...
    const restBonus = this.state.arousal < 0.3 ? BioParametersConfig.metabolism.cortisolDecayRate : 0;
    const totalDecay = basalClearance + restBonus;

    // Net cortisol change per tick
    const netCortisol = (totalStress - totalDecay) * dt;
    this.state.cortisol = Math.max(0, Math.min(BioParametersConfig.limits.maxHormoneLevel, this.state.cortisol + netCortisol));
  }
  ...
  getRiskMultiplier(): number {
    ...
    if (this.state.cortisol > BioParametersConfig.thresholds.panicCortisol) return 0.1; // Panic mode

    const flowState = this.state.attention * this.state.momentum;
    const mood = this.state.valence; // 0-1

    if (flowState > BioParametersConfig.thresholds.flowStateAttention && mood > BioParametersConfig.thresholds.flowStateValence) {
