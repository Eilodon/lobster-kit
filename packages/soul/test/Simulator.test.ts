...
describe('EidolonSimulator: Hardening Body', () => {
    ...
    it('should fail-closed risk matrix when footprint scan fails', async () => {
        const tx: ShadowTransaction = {
            to: '0xTarget',
            data: '0xData',
            account: '0xSender'
        ...
        expect(matrix.footprint.error).toContain('Footprint Scan Failed');
        expect(matrix.allPassed).toBe(false);
    });
