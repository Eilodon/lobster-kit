...
describe('EidolonSwarm (The Hive)', () => {
    ...
    it('should cap and rate-limit external DANGER payloads', async () => {
        agentA = new EidolonSwarm('AGENT_A');
        agentB = new EidolonSwarm('AGENT_B');

        const bus = EidolonBus.getInstance();
        const seen: any[] = [];
        const unsub = bus.subscribe(EidolonEventType.TRAUMA, (event: any) => {
            seen.push(event);
        });

        // Simulate handshake + incoming danger from peer
        (agentB as any).handleMessage({
            sourceAgentId: 'AGENT_A',
            type: 'DISCOVERY',
        ...
        (agentB as any).handleMessage({
            sourceAgentId: 'AGENT_A',
            type: 'DANGER',
            payload: { severity: 100, reason: 'flood' },
            timestamp: Date.now()
        });
        (agentB as any).handleMessage({
            sourceAgentId: 'AGENT_A',
            type: 'DANGER',
            payload: { severity: 100, reason: 'flood-2' },
            timestamp: Date.now()
        });

        expect(seen.length).toBe(1);
        expect(seen[0]?.payload?.severity).toBe(50);
        expect(seen[0]?.payload?.source).toBe('SWARM:AGENT_A');
    ...
    it('should unsubscribe bus listeners on close()', () => {
        const bus = EidolonBus.getInstance();
        agentA = new EidolonSwarm('AGENT_A');
        agentA.close();
        ...
        expect(() => {
            bus.emitEvent({
                ...
                timestamp: Date.now(),
                payload: { severity: 10, reason: 'post-close' }
            });
        }).not.toThrow();
    });
});
