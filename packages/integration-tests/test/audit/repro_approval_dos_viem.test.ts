...
describe('CRITICAL AUDIT: ApprovalRevoker DOS (Viem)', () => {
    ...
    let approvalRevokerArtifact: Artifact | null = null;
    let mockErc20Artifact: Artifact | null = null;
    let isReady = false;
    ...
    beforeAll(async () => {
        ...
        approvalRevokerArtifact = await loadArtifact([
            '../../artifacts/contracts/ApprovalRevoker.sol/ApprovalRevoker.json',
            '../../../../artifacts/contracts/ApprovalRevoker.sol/ApprovalRevoker.json'
        ]);
        mockErc20Artifact = await loadArtifact([
            '../../artifacts/contracts/mocks/MockERC20.sol/MockERC20.json',
            '../../../../artifacts/contracts/mocks/MockERC20.sol/MockERC20.json'
        ]);
        if (!approvalRevokerArtifact || !mockErc20Artifact) {
        ...
        const revokerHash = await client.deployContract({
            ...
            bytecode: approvalRevokerArtifact.bytecode
        });
    ...
    it("Should demonstrate DOS if malicious agent spams flags", async () => {
        if (!isReady || !approvalRevokerArtifact) return;
        try {
            await publicClient.getBlockNumber();
