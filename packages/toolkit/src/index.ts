...
import { getPriceService } from './services/PriceService'; // P3: Shared PriceService
...
export class ClawKit implements IClawKit {
    ...
    constructor(walletClient: ClawKitWalletClient, config: ClawKitConfig) {
        ...
        // FIX P1: Inject Analytics as Price Oracle for DeFi (Thermodynamics)
    ...
    private setupSensoryLinks() {
        ...
        // FIX H2: Honor Privacy Mode
        if (this.config.privacyMode !== 'strict') {
            this.marketStream.start();
        } else {
            console.info('👻 GHOST PROTOCOL: Market Stream disabled (Strict Privacy)');
        }
    }
