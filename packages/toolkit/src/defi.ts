import { PublicClient, parseUnits, formatEther, formatUnits, encodeFunctionData, parseAbi } from 'viem';
...
export class DeFiModule {
  ...
  private handleSwapError(error: unknown) {
    ...
    } else if (message.includes('INSUFFICIENT_INPUT_AMOUNT')) {
  ...
  */
  ...
  async autoCompound(): Promise<{ hash: string; reinvestedAmount: string }> {
    try {
      ...
      const { totalRewards } = await this.harvestAll();
